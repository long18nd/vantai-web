...
const ContactSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      ...
      setToast({ message: "Vui lòng kiểm tra lại thông tin đã nhập.", type: 'error' });
    ...
    try {
      ...
      setToast({ message: "Cảm ơn bạn! Chúng tôi sẽ liên hệ sớm nhất.", type: 'success' });
      ...
      setToast({ message: error.message || "Đã xảy ra lỗi khi gửi yêu cầu. Vui lòng thử lại.", type: 'error' });
    }
  };
  ...
  return (
    <div id="contact-section" className="w-full bg-white py-12 px-4 sm:px-6 lg:px-8 relative">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-1/3 p-4 border rounded-lg shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Tên đơn vị</label>
              <input
                type="text"
                name="customerName"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Tên mặt hàng</label>
              <input
                type="text"
                name="itemName"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Trọng lượng/cân nặng (kg)</label>
              <input
                type="text" 
                name="weight"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Địa chỉ gửi hàng</label>
              <input
                type="text"
                name="fromAddress"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Địa chỉ nhận hàng</label>
              <input
                type="text"
                name="toAddress"
            ...
            <button type="submit" className="w-full bg-amber-600 text-white py-2 rounded mt-2 hover:bg-amber-700 transition-colors">
              Nhận tư vấn ngay
            </button>
          </form>
        </div>
